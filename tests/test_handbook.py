...
SCRIPTS = ROOT / "skill" / "build-project-handbook" / "scripts"
SCAFFOLD = SCRIPTS / "scaffold_handbook.py"
BUILD = SCRIPTS / "build_handbook.py"
VERIFY = SCRIPTS / "verify_handbook.py"
...
class HandbookTest(unittest.TestCase):
    ...
    def test_full_pipeline(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue((target / "pages" / "reading-guide.html").is_file())
    ...
    def test_scaffold_preserves_existing_files(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("preserved 7 existing file(s)", result.stdout)

    def test_unknown_page_link_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            target = Path(temp) / "handbook"
            self.scaffold(target)
            page = target / "content" / "reading-guide.html"
