(() => {
  ...
  const base = window.__DOSSIER_BASE__ || "";
  const index = Array.isArray(window.__DOSSIER_SEARCH__) ? window.__DOSSIER_SEARCH__ : [];
  ...
  function preferredTheme() {
    const saved = localStorage.getItem("ai-pm-dossier-theme");
    if (saved === "light" || saved === "dark") return saved;
    return window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
  }
  function setTheme(value) {
    root.dataset.theme = value;
    localStorage.setItem("ai-pm-dossier-theme", value);
  ...
  if (sidebar) {
    const saved = sessionStorage.getItem("ai-pm-dossier-sidebar-scroll");
    ...
    sidebar.addEventListener("scroll", () => sessionStorage.setItem("ai-pm-dossier-sidebar-scroll", String(sidebar.scrollTop)), { passive: true });
  }
