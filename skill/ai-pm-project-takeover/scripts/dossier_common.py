"""Shared configuration and HTML helpers for AI PM dossier scripts."""
...
REQUIRED_SLUGS = {
    "index", "executive-brief", "reading-guide", "product-context",
    "user-journeys", "capability-map", "ai-system", "data-knowledge",
    "evaluation-guardrails", "metrics-scorecard", "economics-performance",
    "architecture-integrations", "operations-dependencies",
    "current-state-roadmap", "product-debt", "risks-unknowns",
    "stakeholder-agenda", "takeover-plan", "drift-register", "glossary-sources",
}
...
def load_config(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"Invalid dossier.json: {exc}") from exc
    if not isinstance(config, dict):
        raise ValueError("dossier.json must contain an object")
    for field in ("language", "title", "subtitle", "description", "version", "sections"):
        if field not in config:
            raise ValueError(f"dossier.json missing field: {field}")
...
def flatten_pages(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for section_index, section in enumerate(config["sections"]):
        ...
        for raw in raw_pages:
            ...
            if raw.get("status") not in {None, "verified", "supported", "inferred", "unknown"}:
                raise ValueError(f"Page {slug} has invalid status")
            page = dict(raw)
            page.update(
