#!/usr/bin/env python3
"""Verify handbook sources and generated offline output."""
...
from handbook_common import ASSET_REF_RE, PAGE_REF_RE, flatten_pages, load_config
...
def resolve_link(source: Path, root: Path, href: str) -> Path | None:
    ...
    try:
        ...
        return Path("/__outside_handbook__")
    return target
...
def verify(root: Path) -> tuple[list[str], list[str], dict[str, int]]:
    ...
    for page in pages:
        ...
        if not page.get("sources") and not str(page.get("rationale", "")).strip():
            errors.append(f"handbook.json page {page['slug']}: add sources or a rationale")
    ...
    for path in generated:
        ...
        for href in inspector.hrefs:
            ...
            if target is None:
                continue
            if target.as_posix() == "/__outside_handbook__":
                errors.append(f"{label}: link escapes handbook root: {href}")
            elif not target.exists():
                errors.append(f"{label}: broken link: {href}")
        counts["external"] += len(inspector.external_links)
        counts["mermaid"] += inspector.mermaid
        scan_secrets(label, text, errors)
    scan_secrets("handbook.json", (root / "handbook.json").read_text(encoding="utf-8"), errors)
    return errors, warnings, counts


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify an offline project handbook.")
