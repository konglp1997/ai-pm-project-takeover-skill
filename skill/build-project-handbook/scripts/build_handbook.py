#!/usr/bin/env python3
"""Build a searchable offline HTML handbook."""
...
<head>
  ...
  <script>window.__HANDBOOK_BASE__={json.dumps(base)};</script>
</head>
<body>
...
def build(root: Path) -> tuple[int, list[str]]:
    ...
    (assets / "search-index.js").write_text(
        "window.__HANDBOOK_SEARCH__=" + json.dumps(search, ensure_ascii=True, separators=(",", ":")) + ";\n",
        encoding="utf-8",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build an offline HTML project handbook.")
    ...
    try:
        ...
        return 1
    print(f"Built {count} page(s) in {args.root.expanduser().resolve()}")
